...
internal abstract class SegmentWriter : IAsyncDisposable {
    ...
    /// </exception>
    public ValueTask<long> WriteAsync(LogMessage message, ulong sequenceId, CancellationToken cancellationToken = default) => WriteAsync(message, sequenceId, _timeProvider.GetTimestamp(), cancellationToken);
    ...
    ///   The segment writer has been disposed.
    /// </exception>
    internal async ValueTask<long> WriteAsync(LogMessage message, ulong sequenceId, long timestamp, CancellationToken cancellationToken) {
        ...
        var bytesWritten = await WriteMessageCoreAsync(message, sequenceId, timestamp).ConfigureAwait(false);
    ...
    protected abstract ValueTask<long> WriteMessageCoreAsync(LogMessage message, ulong sequenceId, long timestamp);
